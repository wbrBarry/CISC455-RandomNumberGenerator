	...
	for ( i=0; i<tp.numOfBitStreams; i++ ) {
		...
		for ( j=0; j<tp.n; j++ ) {
			if ( fscanf(fp, "%1d", &bit) == EOF ) {
				printf("ERROR:  where? Insufficient data in file %s.  %d bits were read.\n", streamFile, bitsRead);
				fclose(fp);
				free(epsilon);
				return;
			}
...
void
openOutputStreams(int option, char* folderName)
{
	int		i, numOfBitStreams, numOfOpenFiles = 0;
	char	freqfn[200], summaryfn[200], statsDir[200], resultsDir[200];
	
	sprintf(freqfn, "%s/%s/freq.txt",folderName, generatorDir[option]);
	if ( (freqfp = fopen(freqfn, "w")) == NULL ) {
		printf("\t\tMAIN:  Could not open freq file: <%s>", freqfn);
		exit(-1);
	}
	sprintf(summaryfn, "%s/%s/finalAnalysisReport.txt",folderName, generatorDir[option]);
	...
	for( i=1; i<=NUMOFTESTS; i++ ) {
		if ( testVector[i] == 1 ) {
			...
			sprintf(resultsDir, "%s/%s/%s/results.txt", folderName,generatorDir[option], testNames[i]);
