...
void	postProcessResults(int option, char *dirName);
...
{
	int		i;
	int		option = 0;			/* TEMPLATE LENGTH/STREAM LENGTH/GENERATOR*/
	char	*streamFile;	/* STREAM FILENAME     */
	

	if ( argc != 4 ) {
		printf("Usage: %s <stream length> <numOfStream> <filename> <>\n", argv[0]);
		printf("   <stream length> is the length of the individual bit stream(s) to be processed\n");
		return 0;
	}

	tp.n = atoi(argv[1]);
	tp.numOfBitStreams = atoi(argv[2]);
	streamFile = (char *) malloc(strlen(argv[3]) + 1);
	strcpy(streamFile, argv[3]);
	char* fileName = (char *) malloc(strlen(argv[3]) - 4 + 1);
	strncpy(fileName, streamFile, strlen(argv[3]) - 4);
	fileName[strlen(argv[3]) - 4] = '\0';
	printf("%s\n", fileName);

	// sprintf(*streamFile, "%s", argv[2]);
	...
	// fixParameters();
	openOutputStreams(option, fileName);
	...
	fprintf(summary, "------------------------------------------------------------------------------\n");
	postProcessResults(option, fileName);
	fclose(summary);
	free(streamFile);
	free(fileName);
...
void
postProcessResults(int option, char *dirName)
{
		...
		if ( testVector[i] ) {
			...
			if ( ((i == TEST_CUSUM) && testVector[TEST_CUSUM] ) ||
				...
				for ( k=0; k<numOfFiles; k++ ) {
					if ( k < 10 )
						sprintf(s, "%s/%s/%s/data%1d.txt", dirName, generatorDir[option], testNames[i], k+1);

					else if ( k < 100 )
					
						sprintf(s, "%s/%s/%s/data%2d.txt", dirName, generatorDir[option], testNames[i], k+1);
					else
						sprintf(s, "%s/%s/%s/data%3d.txt", dirName,generatorDir[option], testNames[i], k+1);
				...
				sprintf(s, "%s/%s/%s/results.txt", dirName,generatorDir[option], testNames[i]);
				generalSampleSize = computeMetrics(s,i);
			}
		}
	}
